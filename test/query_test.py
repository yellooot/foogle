...
class QueryTests(unittest.TestCase):
    ...
    def test_invalid_query(self):
        for query_string in ["not", "apple and",
                             "apple and (orange or banana) or", "or not and"]:
            self._test_raising_bad_query_error(query_string)

    def test_mismatched_parentheses(self):
        for query_string in ["apple and (orange or banana", "(apple))",
                             "((apple)"]:
            self._test_raising_bad_query_error(query_string)
    ...
    def test_inexact_forms(self):
        self._test_inexact_form('кошечка',
                                {'кошечке', 'кошечку', 'кошечки', 'кошечкой',
                                 'кошечек', 'кошечка', 'кошечкам', 'кошечках',
                                 'кошечкою', 'кошечками'})
        self._test_inexact_form('cat',
                                {'catties', 'catty', 'cat', 'cats', 'catting',
    ...
    def _test_inexact_form(self, word, expected_word_forms):
        ...
        self.assertEqual(set(query[:expected_word_forms_amount]),
                         expected_word_forms)
        self.assertEqual(query[expected_word_forms_amount:],
                         expected_operators)
